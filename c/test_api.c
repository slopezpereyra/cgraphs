...
/**
 * @brief Tests the functionality of `edgeIndex` to get correct edge position in
 * the array.
 */
void testEdgeIndex() {
...
/**
 * @brief Tests the removal of an edge and checks if degrees and edges are
 * correctly updated.
 */
void testRemoveEdge() {
...
/**
 * @brief Tests neighbor functionality to ensure `isNeighbour` detects
...
/**
 * @brief Tests that the degree values are correctly calculated and stored in
 * the graph.
 */
void testDegree() {
...
/**
 * @brief Tests the edge comparison function to ensure edges are ordered
 * correctly.
 */
...
/**
 * @brief Tests the initialization of graph from an input file and checks for
 * graph creation. Requires a valid input file in Penazzi format for full
 * functionality.
 */
void test_readGraph() {
  ...
  assert(Δ(W) == 2);
  bool tw =
      isNeighbour(0, 1, W) && isNeighbour(1, 2, W) && isNeighbour(2, 3, W);
  bool tw2 = *(getEdge(0, 1, W).w) == 1 && *(getEdge(1, 2, W).w) == 10 &&
             *(getEdge(2, 3, W).w) == 20;
  assert(tw);
  assert(tw2);
  dumpGraph(W);
...
int main() {
  ...
  testColors();
  testCompareEdges();
  // Note: test_readGraph requires an actual file input for complete
  // verification.
